 ...
 * Scan ranked top-gainers from highest to lowest; skip blocked/active/invalid.
 */
export function selectReplacementSymbols(params: {
  ...
  for (const symbol of params.rankedSymbols) {
    ...
    if (params.blockedSymbols.has(symbol)) continue;
...
export function calcBlockUntil(blockedAt: Date, durationHours: number): Date {
  return new Date(blockedAt.getTime() + durationHours * 60 * 60 * 1000);
}

export function isBlockActive(blockedUntil: Date, now: Date = new Date()): boolean {
  return now.getTime() < blockedUntil.getTime();
}
